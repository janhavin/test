...
public class TestBase {
	...
	private static final String executionType = applicationProperty.getProperty("executionType");
	
	
//	@Optional("SAUCE") String executionType
	@BeforeTest
	//@Parameters("executionType")
	protected void beforeTest(ITestContext context) {try{
				

				Browser browser = new Browser(
						applicationProperty.getProperty("browserName"),
						applicationProperty.getProperty("browserVersion"),
						Platform.WINDOWS, applicationProperty.getProperty("lang") );
				...
				LOGG.error("Error occured in Before Test method of the base class", e );
			}
	}
	...
	protected void afterTest(ITestContext context) {
		...
		context.removeAttribute(context.getCurrentXmlTest().getName());
			...
			LOGG.debug(Utilities.getCurrentThreadId() + "Nothing to close in after test. Driver instance is null:");
	...
	protected void afterMethod(ITestResult result) throws IOException {
		...
		FileUtils.copyFile(screenshot, new File("./screenshots/"
				+ screenShotName));
		LOGG.debug(Utilities.getCurrentThreadId() + "Screenshot taken for test : " +result.getTestName() );
		 }
		LOGG.debug(Utilities.getCurrentThreadId() + "Screenshot not taken as WebDriver object is null");
